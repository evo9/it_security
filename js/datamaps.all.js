(function () {
            ...
            if (key != 'defaultFill') {
                var legend_item = legend.selectAll('g.legend_block:last-child').append('g')
                    ...
                    .on('mouseover', function (d) {
                        ...
                        self.legendMouseover($this, svg);
                    })
                    .on('mouseout', function () {
                        var $this = d3.select(this);

                        self.legendMouseout($this, fillData, svg);
                    });

                legend_item.append('rect')
                    .attr('class', 'legend_color level_' + key)
                    .attr('width', 10)
                    .attr('height', 10)
                    .attr('x', x)
                    .attr('y', y)
    ...
    function drawSubunits(data) {
        ...
        if (subunits.empty()) {
            subunits = this.addLayer('datamaps-subunits', null, false);
        }
        ...
        layer
            ...
            .on('click', function (datum) {
                ...
                if (options.popupOnClick) {
                    self.bubbleClick($this, datum, options, svg);
                }
            })
            .transition().duration(400)
            .attr('r', function (datum) {
                return val(datum.bubblesRadius, options.bubblesRadius, datum);
            })
            .attr('class', function(datum) {
                var cls = 'datamaps-bubble ';
    ...
    Datamap.prototype.draw = function () {
        ...
        function draw(data) {
            ...
            if (self.options.bubblesConfig.popupOnClick) {
                tooltipWrap = d3.select(self.options.element).append('div')
    ...
    Datamap.prototype.bubbleClick = function (element, d, options) {
        var self = this;
        d3.selectAll('.tooltip-wrap').style('display', 'none');
        d3.select(self.svg[0][0].parentNode).select('.tooltip-wrap')
            .html(function () {
                var data = JSON.parse(element.attr('data-info'));
                return options.popupTemplate(d, data);
            });
        var position = d3.mouse(self.options.element);
        var l = position[0] - 125 + 'px';
        var t = position[1] - 210 + 'px';
        ...
        d3.select('a.close_tooltip_button').on('click', function () {
            d3.selectAll('.tooltip-wrap').style('display', 'none');
        });
    ...
    Datamap.prototype.legendMouseover = function(element) {
        var self = this;

        var level = element.attr('data-level');

        d3.select(self.svg[0][0].parentNode).selectAll('.datamaps-arc, .datamaps-bubble')
            .style('opacity', 0.15);
        d3.select(self.svg[0][0].parentNode).selectAll('.datamaps-arc.' + level)
            .style('opacity', 1);
        d3.select(self.svg[0][0].parentNode).selectAll('.datamaps-bubble.' + level)
            .style('opacity', 0.75);
    ...
    Datamap.prototype.legendMouseout = function(element, fillData) {
        var self = this;

        d3.select(self.svg[0][0].parentNode).selectAll('.datamaps-arc')
            .style('opacity', 1);
        d3.select(self.svg[0][0].parentNode).selectAll('.datamaps-bubble')
            .style('opacity', 0.75);
